              ...
              taxasFiltradas.map((item) => {
                ...
                return (
                  <tr key={item.id} className={`hover:bg-slate-50/50 transition-colors ${isChecked ? 'bg-emerald-50/40' : ''}`}>
                    ...
                    <td className="p-5 text-slate-500 font-bold text-xs">{item.mes_referencia} <br/><span className="text-[10px] text-slate-400 font-medium uppercase">{dataVencimento}</span></td>
