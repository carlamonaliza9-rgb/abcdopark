    ...
    if (tipoPagamento === "acordo") {
      ...
      for (let i = 0; i < qtdParcelas; i++) {
        ...
        parcelasParaInserir.push({
          aluno_id: alunoSelecionado.id, tipo: 'acordo', descricao: `Acordo Financeiro - Parcela ${i + 1}/${qtdParcelas}`,
          valor_total: valorParcela, valor_pago: 0, status: 'pendente', data_pagamento: dataVenc.toISOString().split('T')[0],
          detalhes_metodos: {}
        });
      }
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="flex gap-2 mb-6">
        <button onClick={() => setAbaAtiva("todos")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${abaAtiva === "todos" ? "bg-slate-700 text-white shadow-sm" : "bg-white text-gray-600 hover:bg-gray-100 border border-gray-200/60"}`}>
          📂 Todos ({alunosConsolidados.length})
        </button>
        <button onClick={() => setAbaAtiva("dividas")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${abaAtiva === "dividas" ? "bg-rose-600 text-white shadow-sm" : "bg-white text-gray-600 hover:bg-gray-100 border border-gray-200/60"}`}>
          🔴 Apenas Devedores ({alunosConsolidados.filter(i => i.saldoFinal < 0).length})
        </button>
        <button onClick={() => setAbaAtiva("creditos")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${abaAtiva === "creditos" ? "bg-emerald-600 text-white shadow-sm" : "bg-white text-gray-600 hover:bg-gray-100 border border-gray-200/60"}`}>
                    ...
                    <Fragment key={item.id}>
                      <tr className="hover:bg-slate-50/40 transition-colors font-medium">
                        <td className="p-4 text-center">
                          <button onClick={() => toggleAluno(item.id)} className="text-slate-400 hover:text-slate-600 transition-colors focus:outline-none">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`w-5 h-5 transform transition-transform duration-200 ${expandido ? "rotate-180" : ""}`}>
                              <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clipRule="evenodd" />
                            </svg>
                          </button>
                        </td>
                        <td onClick={() => toggleAluno(item.id)} className="p-4 font-bold text-slate-800 uppercase cursor-pointer hover:text-blue-600 transition-colors truncate">{item.nome}</td>
                        ...
                        <td className="p-4 text-center">
                          <button onClick={() => gerarPDFExtratoAluno(item)} className="bg-slate-100 text-slate-700 hover:bg-slate-200 font-bold px-3 py-1.5 rounded-lg text-xs transition-all border border-slate-200/40">🖨️ PDF Extrato</button>
                        </td>
                      </tr>
                      {expandido && (
                              ...
                              {item.dividas.map((div) => (
                                <div key={div.id} className="flex justify-between items-center py-2.5 px-4 bg-white border border-gray-100 rounded-xl text-xs shadow-sm hover:border-gray-200 transition-all">
                                  <div>
                                    ...
                                    <p className="text-[10px] text-slate-400 mt-0.5">Lançamento: <b className="text-slate-600 font-semibold">{div.dataCriacao}</b> | Última Movimentação: <b className="text-slate-600 font-semibold">{div.dataPagamento}</b> | Total: R$ {div.valorTotal.toFixed(2)} | Pago: R$ {div.valorPago.toFixed(2)}</p>
                                  </div>
      ...
      <ModalPagamento 
        ...
        descricaoOutro={descricaoOutro} setDescricaoOutro={setDescricaoOutro}
        pagamentosMetodos={pagamentosMetodos} setPagamentosMetodos={setPagamentosMetodos} onConfirmar={confirmarPagamento} editando={true}
      />
    </div>
  );
}
...
// ============================================================================
// COMPONENTE PRINCIPAL MESTRE (Página Base com Tabs)
// ============================================================================
export default function ControleFinanceiroUnificadoPage() {
  const router = useRouter();
  const [visaoAtiva, setVisaoAtiva] = useState<"mensalidades" | "saldos">("mensalidades");
  ...
  return (
    <div className="w-full bg-slate-50 min-h-screen font-sans antialiased text-slate-800 pb-24 md:pb-8 flex flex-col">
      {/* HEADER DE TABS MASTER */}
      <div className="max-w-7xl w-full mx-auto pt-6 px-4 md:px-8 mb-4">
