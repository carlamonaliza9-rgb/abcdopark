...
  }
  
  // 1. Extração do ano contido explicitamente na Descrição (Ex: "Janeiro/2026")
  const anoMatch = desc.match(/(20\d{2})/);
  let ano = anoMatch ? anoMatch[0] : null;
  
  // 2. Extração pela verdadeira competência (Data de Vencimento)
...
  }
  
  // 3. Lógica de Antecipação de Matrícula (Se pago no fim do ano anterior)
...
      
      // Pagou entre Out-Dez referente a Jan-Abr? É do ano letivo seguinte!
...
      
      // Receitas que caíram fisicamente neste mês (Para a barra de Lucro/Extrato)
...
      
      // Todas as pendências financeiras do sistema
...
      // === A GRANDE MUDANÇA: Busca todas as mensalidades para validar a inteligência ===
      const { data: historicoGeralDB } = await supabase.from('historico_pagamentos')
        .select('*')
        .eq('tipo', 'mensalidade')
        .neq('status', 'cancelado')
        .neq('status', 'estornado');
      
      // Proteção estrita do TypeScript contra nulos
      const historicoGeral = historicoGeralDB || [];

      const nomeMesReferencia = mesesAno[parseInt(mes) - 1].toLowerCase();

      // Monta a lista de alunos que JÁ PAGARAM essa competência usando o Motor Inteligente
      const idsPagosNestaReferencia = historicoGeral.filter((p: any) => {
        if (p.status !== 'pago' && p.status !== 'renegociado' && p.status !== 'parcial') return false;
        
        const ref = extrairMesAnoInteligente(p, ano);
        return ref.mes === nomeMesReferencia && ref.ano === ano;
      }).map((p: any) => p.aluno_id);

      // Une as listas para gerar a tabela da tela
      const mapaPgtos = new Map();
      pgtosMes.forEach((p: any) => mapaPgtos.set(p.id, p));
      pgtosPendentes.forEach((p: any) => mapaPgtos.set(p.id, p));
      // Garante que o pagamento antecipado (ex: de 2025) vá para a tela se pertencer a esta competência
      historicoGeral.forEach((p: any) => {
        const ref = extrairMesAnoInteligente(p, ano);
        if (ref.mes === nomeMesReferencia && ref.ano === ano && !mapaPgtos.has(p.id)) {
          mapaPgtos.set(p.id, p);
        }
      });
...
      if (listaAlunos) {
        // TABELA: O aluno pagou essa competência baseada no Motor Inteligente?
...
          
          // Se não pagou, verifica rigorosamente o atraso
...
          
          return hoje > dataVencObj ? { ...aluno, status: 'atrasado' } : { ...aluno, status: 'pendente' };
        });
        
        setAlunos(ordenados);
        
...
        
        const mensalidadesDesteMes = pgtosFiltrados.filter((p: any) => {
...
        // =======================================================================
        // DÍVIDA ATIVA: Filtro rigoroso para bloquear pendências futuras falsas
        // =======================================================================
        const todasPendenciasExtras = pgtosFiltrados.filter((p: any) => {
          if (p.status !== 'pendente' && p.status !== 'parcial' && p.status !== 'atrasado') return false;
          
          if (p.tipo === 'mensalidade') {
            const dataVenc = new Date(p.data_vencimento || p.vencimento || p.data_pagamento || hoje);
            dataVenc.setHours(0,0,0,0);
            if (hoje <= dataVenc) return false; // Bloqueia cobrança se não passou do vencimento
            
            // Bloqueia cobrança se o Motor identificar que aquela competência JÁ FOI PAGA em outra data
            const ref = extrairMesAnoInteligente(p, ano);
            if (idsPagosNestaReferencia.includes(p.aluno_id) && ref.mes === nomeMesReferencia && ref.ano === ano) return false;
          }
          return true;
        });
...
    const pgtosEfetuadosEsteMes = listaReceitasDetalhada.filter((p: any) => p.data_pagamento && p.data_pagamento >= dataInicio && p.data_pagamento <= dataFim && p.status !== 'pendente' && p.status !== 'atrasado');
...
                  </h3>
                  <p className="text-sm text-slate-500 mt-1">Maiores saldos devedores (Mensalidades atrasadas e pendências ativas)</p>
                </div>
...
        mesFiltro={mesFiltro} 
        listaGastos={listaReceitasDetalhada.filter(r => r.status === 'pago' || r.status === 'parcial').map((r: any) => {
