...
export default function FinanceiroPage() {
  ...
  if (carregando) return <div className="p-10 text-center text-sm sm:text-base md:text-[10px] font-black uppercase text-slate-300 animate-pulse tracking-widest">Sincronizando fluxo financeiro...</div>;
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10 w-full px-2">
      <header className="mb-10">
        ...
        <p className="text-sm md:text-[9px] font-bold uppercase text-slate-400 tracking-widest mt-2 italic">Acompanhamento: <span className="text-indigo-600 font-black">{aluno?.nome}</span></p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
        <div className="bg-indigo-600 rounded-[2rem] p-6 text-white flex items-center gap-5 shadow-xl shadow-indigo-100">
          ...
          <div>
            <p className="text-xs sm:text-sm md:text-[8px] font-black uppercase tracking-[0.2em] opacity-70">Valor da Mensalidade</p>
        ...
        <div className="bg-white border border-slate-100 rounded-[2rem] p-6 flex items-center gap-5 shadow-sm">
          ...
          <div>
            <p className="text-xs sm:text-sm md:text-[8px] font-black uppercase tracking-[0.2em] text-slate-400">Dia de Vencimento</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div key={idx} className={`bg-white rounded-[2.5rem] border p-6 transition-all ${m.status === 'pago' ? 'border-slate-100 opacity-90' : 'border-indigo-50 shadow-sm'}`}>
                <div className="flex flex-col md:flex-row justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm sm:text-base md:text-[11px] font-black text-slate-700 uppercase">{m.mes}</p>
                      <p className="text-xs sm:text-sm md:text-[8px] font-bold text-slate-400 uppercase tracking-widest">Vencimento: {formatarData(m.vencimento)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6">
                    <div className="text-right">
                      ...
                      <span className={`text-xs md:text-[7px] font-black uppercase px-2 py-0.5 rounded-md ${m.status === 'pago' ? 'bg-emerald-50 text-emerald-600' : (m.status === 'atrasado' ? 'bg-rose-50 text-rose-600 flex items-center gap-1' : 'bg-amber-50 text-amber-600')}`}>
                        {m.status === 'pago' ? 'Liquidado' : (m.status === 'atrasado' ? <> <AlertCircle size={8}/> Vencido </> : 'Pendente')}
                      </span>
                    </div>

                    {m.status !== "pago" && (
                      <button onClick={() => handlePagarPix(m.valor, `Mensalidade ${m.mes}`)} className="bg-slate-900 text-white px-5 py-3 rounded-xl font-black text-xs sm:text-sm md:text-[9px] uppercase tracking-widest hover:bg-indigo-600 active:scale-95 transition-all flex items-center gap-2">
                ...
                {m.status === "pago" && (
                  <div className="mt-5 pt-5 border-t border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50/50 p-4 rounded-2xl">
                    <div>
                      <p className="text-xs md:text-[7px] font-black text-slate-400 uppercase tracking-widest">Data do Pagamento</p>
                      <p className="text-sm md:text-[10px] font-bold text-slate-600">{formatarData(m.data_pagamento)}</p>
                    </div>
                    <div>
                      <p className="text-xs md:text-[7px] font-black text-slate-400 uppercase tracking-widest">Forma / Valor</p>
                    ...
                    <div className="col-span-2 flex justify-end">
                      {m.comprovante_url && (
                        <a href={m.comprovante_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm md:text-[9px] font-black text-indigo-600 uppercase hover:bg-indigo-50 shadow-sm">
        ...
        <div className="lg:col-span-4 space-y-6">
              ...
              {eventosTaxas.length > 0 ? eventosTaxas.map((e, idx) => (
                <div key={idx} className={`p-5 rounded-2xl border transition-all ${e.pago ? 'bg-slate-50 border-slate-100' : 'bg-amber-50/30 border-amber-100'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <p className="text-sm sm:text-base md:text-[10px] font-black text-slate-700 uppercase leading-tight">{e.nome}</p>
                      <p className="text-xs md:text-[7px] font-bold text-slate-400 uppercase mt-1">Valor: {formatarMoeda(e.valor_unitario)}</p>
                    </div>
                  ...
                  {e.pago ? (
                    <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                      <span className="text-xs md:text-[7px] font-black text-emerald-600 uppercase">Pago: {formatarData(e.data_pagamento)}</span>
                    ...
                    <button onClick={() => handlePagarPix(e.valor_unitario, e.nome)} className="w-full bg-indigo-600 text-white py-2.5 rounded-xl text-xs sm:text-sm md:text-[8px] font-black uppercase shadow-md hover:bg-indigo-700">Pagar Taxa</button>
                  )}
                </div>
              )) : (
                <div className="text-center py-6">
                  <PartyPopper size={24} className="mx-auto mb-2 text-slate-200" />
                  <p className="text-sm md:text-[9px] font-black text-slate-300 uppercase tracking-widest">Nenhuma taxa extra.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-center text-white shadow-xl">
            ...
            <p className="text-xs sm:text-sm md:text-[10px] font-black uppercase tracking-widest mb-6 leading-relaxed">Dúvidas financeiras? Fale com a secretaria.</p>
            <button className="w-full bg-white text-slate-900 py-4 rounded-2xl font-black text-xs sm:text-sm md:text-[10px] uppercase tracking-widest hover:bg-indigo-50 transition-all">WhatsApp Suporte</button>
          </div>
        </div>
      </div>
    </div>
