...
export default function FichaAlunoPage() {
  ...
  const LinhaInfo = ({ icone: Icon, label, value }: { icone: any; label: string; value: string }) => (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-4 border-b border-slate-50 last:border-0 group transition-all hover:bg-slate-50/50 px-2 rounded-lg gap-2 sm:gap-4">
      <div className="flex items-center gap-4">
        <div className="text-slate-300 group-hover:text-indigo-500 transition-colors flex-shrink-0">
          <Icon size={18} strokeWidth={2.5} />
        </div>
        <span className="text-[10px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest">{label}</span>
      </div>
      <span className="text-sm md:text-xs font-bold text-slate-700 uppercase sm:text-right break-words">{value || "---"}</span>
    </div>
  );

  if (carregando) return <div className="p-10 text-center text-sm md:text-[10px] font-black uppercase text-slate-300 animate-pulse tracking-widest">Sincronizando prontuário...</div>;
  if (!aluno) return <div className="p-10 text-center text-sm md:text-[10px] font-black uppercase text-rose-400 font-bold">Registro não localizado.</div>;
          ...
          <div className="bg-white w-full max-w-2xl rounded-[3rem] p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-[10px] md:text-[9px] font-bold text-rose-500 uppercase tracking-widest flex items-center gap-1 mt-1">
                ...
                return (
                  <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-slate-50 rounded-[1.8rem] border border-slate-100 hover:border-rose-200 transition-all group gap-4">
                    <div className="flex flex-col">
                      <span className="text-sm md:text-[10px] font-black text-rose-600 uppercase leading-tight">{doc}</span>
                      <span className={`text-[10px] md:text-[8px] font-bold uppercase mt-1 ${docExistente ? 'text-green-500' : 'text-slate-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-50">
            <h2 className="text-sm md:text-[11px] font-black text-indigo-600 uppercase tracking-wider sm:tracking-[0.3em] mb-6 flex items-center gap-2"><Fingerprint size={16} /> Identificação do Aluno</h2>
            <div className="flex flex-col">
          ...
          {/* RESPONSÁVEIS */}
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-50">
            <h2 className="text-sm md:text-[11px] font-black text-indigo-600 uppercase tracking-wider sm:tracking-[0.3em] mb-6 flex items-center gap-2"><Phone size={16} /> Responsáveis Legais</h2>
            <div className="flex flex-col">
              ...
              <div className="my-6 border-t border-slate-100 relative">
                <span className="absolute left-1/2 -top-2 -translate-x-1/2 bg-white px-4 text-[10px] md:text-[7px] font-black text-slate-300 uppercase tracking-[0.4em]">Responsável 2</span>
              </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-indigo-600 rounded-[2.5rem] p-6 sm:p-8 text-white shadow-2xl shadow-indigo-200 group transition-all hover:-translate-y-1">
            ...
            <p className="text-indigo-100 text-sm md:text-[10px] font-bold uppercase tracking-widest leading-relaxed mb-8">Contratos, RG, CPF e declarações digitalizadas.</p>
            <button onClick={() => setModalAberto(true)} className="w-full bg-white text-indigo-600 py-4 rounded-2xl font-black text-xs md:text-[10px] uppercase tracking-[0.2em] shadow-lg active:scale-95 transition-all">Abrir Documentação</button>
          </div>
          
          <div className="bg-slate-50 border border-slate-100 rounded-[2.5rem] p-6 sm:p-8 text-center">
            <p className="text-[11px] md:text-[9px] text-slate-400 font-bold uppercase leading-relaxed italic">
