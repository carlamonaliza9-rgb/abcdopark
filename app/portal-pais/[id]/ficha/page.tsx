  ...
  const LinhaInfo = ({ icone: Icon, label, value }: { icone: any; label: string; value: string }) => (
    <div className="flex items-center justify-between py-4 border-b border-slate-50 last:border-0 group transition-all hover:bg-slate-50/50 px-2 rounded-lg gap-4">
      <div className="flex items-center gap-4">
        ...
        <span className="text-sm md:text-[9px] font-black text-slate-400 uppercase tracking-wider sm:tracking-widest">{label}</span>
      </div>
      <span className="text-sm sm:text-base md:text-xs font-bold text-slate-700 uppercase text-right break-words">{value || "---"}</span>
    </div>
  );

  if (carregando) return <div className="p-10 text-center text-sm sm:text-base md:text-[10px] font-black uppercase text-slate-300 animate-pulse tracking-widest">Sincronizando prontuário...</div>;
  if (!aluno) return <div className="p-10 text-center text-sm sm:text-base md:text-[10px] font-black uppercase text-rose-400 font-bold">Registro não localizado.</div>;
          ...
          <div className="bg-white w-full max-w-2xl rounded-[3rem] p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm md:text-[9px] font-bold text-rose-500 uppercase tracking-widest flex items-center gap-1">
                ...
                return (
                  <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-slate-50 rounded-[1.8rem] border border-slate-100 hover:border-rose-200 transition-all group gap-4">
                    <div className="flex flex-col">
                      <span className="text-sm sm:text-base md:text-[10px] font-black text-rose-600 uppercase leading-tight">{doc}</span>
                      <span className={`text-xs sm:text-sm md:text-[8px] font-bold uppercase mt-1 ${docExistente ? 'text-green-500' : 'text-slate-400'}`}>
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      {docExistente && (
                        <a href={docExistente.arquivo_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-black text-xs sm:text-sm md:text-[9px] uppercase tracking-wider bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
                      ...
                      <label className="cursor-pointer">
                        ...
                        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-black text-xs sm:text-sm md:text-[9px] uppercase tracking-wider transition-all ${enviando === doc ? 'bg-slate-200 text-slate-400' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md shadow-indigo-100'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-50">
            <h2 className="text-sm sm:text-base md:text-[11px] font-black text-indigo-600 uppercase tracking-wider sm:tracking-[0.3em] mb-6 flex items-center gap-2"><Fingerprint size={14} /> Identificação do Aluno</h2>
            <div className="flex flex-col">
          ...
          {/* SAÚDE */}
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-50">
            <h2 className="text-sm sm:text-base md:text-[11px] font-black text-rose-600 uppercase tracking-wider sm:tracking-[0.3em] mb-6 flex items-center gap-2"><Stethoscope size={14} /> Informações de Saúde</h2>
            <div className="flex flex-col">
          ...
          {/* ENDEREÇO */}
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-50">
            <h2 className="text-sm sm:text-base md:text-[11px] font-black text-indigo-600 uppercase tracking-wider sm:tracking-[0.3em] mb-6 flex items-center gap-2"><MapPin size={14} /> Localização de Residência</h2>
            <div className="flex flex-col">
          ...
          {/* RESPONSÁVEIS */}
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-50">
            <h2 className="text-sm sm:text-base md:text-[11px] font-black text-indigo-600 uppercase tracking-wider sm:tracking-[0.3em] mb-6 flex items-center gap-2"><Phone size={14} /> Responsáveis Legais</h2>
            <div className="flex flex-col">
              ...
              <div className="my-6 border-t border-slate-100 relative">
                <span className="absolute left-1/2 -top-2 -translate-x-1/2 bg-white px-4 text-xs md:text-[7px] font-black text-slate-300 uppercase tracking-[0.4em]">Responsável 2</span>
              </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-indigo-600 rounded-[2.5rem] p-6 sm:p-8 text-white shadow-2xl shadow-indigo-200 group transition-all hover:-translate-y-1">
            ...
            <p className="text-indigo-100 text-xs sm:text-sm md:text-[10px] font-bold uppercase tracking-widest leading-relaxed mb-8">Contratos, RG, CPF e declarações digitalizadas.</p>
            <button onClick={() => setModalAberto(true)} className="w-full bg-white text-indigo-600 py-4 rounded-2xl font-black text-xs sm:text-sm md:text-[10px] uppercase tracking-[0.2em] shadow-lg active:scale-95 transition-all">Abrir Documentação</button>
          </div>
          
          <div className="bg-slate-50 border border-slate-100 rounded-[2.5rem] p-6 sm:p-8 text-center">
            <p className="text-sm md:text-[9px] text-slate-400 font-bold uppercase leading-relaxed italic">
