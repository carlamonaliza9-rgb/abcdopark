...
export default function DashboardAluno() {
  ...
  const renderEstrelas = (media: number) => Array.from({ length: 5 }).map((_, i) => <Star key={i} size={22} className={i < Math.round(media) ? "fill-yellow-400 text-yellow-400" : "text-slate-200"} />);
