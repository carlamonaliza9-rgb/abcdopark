...
export default function CalendarioPaisPage() {
  ...
  if (carregando) return <div className="p-10 text-center text-[10px] font-black uppercase text-slate-300 animate-pulse">Carregando Calendário...</div>;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {meses.map((mesNome, index) => (
          <div key={index} className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-50 flex flex-col h-full">
            <h3 className="text-[10px] font-black text-indigo-600 border-b border-slate-50 pb-2 mb-3 uppercase tracking-widest">
              {mesNome}
            </h3>
              ...
              {eventos.filter(ev => new Date(ev.data + "T12:00:00").getUTCMonth() === index).length > 0 ? (
                eventos.filter(ev => new Date(ev.data + "T12:00:00").getUTCMonth() === index).map((ev, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[8px] font-black block mb-0.5" style={{ color: estilo.color }}>
                        DIA {extrairDiaUTC(ev.data)}
                      </span>
                      <span className="text-[10px] font-bold text-slate-700 leading-tight block uppercase">
                        {ev.titulo}
                      </span>
                    </div>
                  );
                ...
                <p className="text-[9px] text-slate-300 font-bold uppercase italic mt-1">Sem eventos</p>
              )}
            </div>
          </div>
        ))}
