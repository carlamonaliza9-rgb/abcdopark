...
export default function CalendarioPaisPage() {
  ...
  if (carregando) return <div className="p-10 text-center text-sm md:text-[10px] font-black uppercase text-slate-300 animate-pulse tracking-widest">Carregando Calendário...</div>;
  ...
  return (
    <div className="animate-in fade-in duration-500 w-full px-2 pb-10">
      <header className="mb-8 border-b border-slate-100 pb-8">
        <h1 className="text-2xl md:text-3xl font-black text-slate-800 uppercase tracking-tight italic">Calendário Escolar</h1>
        <p className="text-xs md:text-[9px] font-bold uppercase text-slate-400 tracking-widest mt-1">
          Acompanhe as datas e eventos da ABC do Park
        </p>
      </header>

      {/* GRID DE MESES: Mantida a lógica responsiva para empilhar no celular */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {mesesOrdenados.map(({ mesNome, originalIndex }) => (
          <div key={originalIndex} className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-50 flex flex-col h-full">
            <h3 className="text-sm md:text-[10px] font-black text-indigo-600 border-b border-slate-50 pb-2 mb-3 uppercase tracking-widest flex items-center justify-between">
              <span>{mesNome}</span>
              {originalIndex < mesAtualIndex && (
                <span className="text-[10px] md:text-[9px] text-slate-300 font-bold lowercase italic tracking-normal">(passado)</span>
              )}
            </h3>
              ...
              {eventos.filter(ev => new Date(ev.data + "T12:00:00").getUTCMonth() === originalIndex).length > 0 ? (
                eventos.filter(ev => new Date(ev.data + "T12:00:00").getUTCMonth() === originalIndex).map((ev, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[10px] md:text-[8px] font-black block mb-0.5 uppercase tracking-widest" style={{ color: estilo.color }}>
                        DIA {extrairDiaUTC(ev.data)}
                      </span>
                      <span className="text-xs md:text-[10px] font-bold text-slate-700 leading-tight block uppercase">
                        {ev.titulo}
                      </span>
                    </div>
                  );
                ...
                <p className="text-[10px] md:text-[9px] text-slate-300 font-bold uppercase italic mt-1">Sem eventos</p>
              )}
            </div>
          </div>
        ))}
