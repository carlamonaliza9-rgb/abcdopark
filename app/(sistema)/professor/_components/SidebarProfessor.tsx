...
import { Home, BookOpen, BarChart3, ClipboardCheck, LogOut, User, BrainCircuit } from "lucide-react";
...
export function SidebarProfessor() {
  ...
  const menuItems = [
    ...
    { name: "Pareceres", icon: BrainCircuit, path: "/professor/avancos-dificuldades" },
  ];
  ...
  return (
    <>
      ...
      {/* ========================================================= */}
      {/* Adicionado overflow-x-auto e hide-scrollbar para permitir scroll se os botões não couberem na tela */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-200/60 px-2 pb-4 pt-3 flex items-center justify-start sm:justify-around z-[9999] shadow-[0_-10px_40px_rgba(0,0,0,0.06)] min-h-[90px] overflow-x-auto hide-scrollbar">
        {menuItems.map((item) => {
          const isActive = pathname === item.path || pathname.startsWith(`${item.path}/`);
          return (
            <Link
              key={item.name}
              href={item.path}
              className="flex flex-col items-center justify-center min-w-[75px] flex-1 py-1 rounded-2xl relative group"
            >
        ...
        <button onClick={handleLogout} className="flex flex-col items-center justify-center min-w-[75px] flex-1 py-1 rounded-2xl group active:opacity-70 transition-opacity">
