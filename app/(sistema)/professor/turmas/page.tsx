...
export default function TurmasProfessorPage() {
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50/50 p-4 md:p-8 lg:p-10 font-sans pb-32 animate-in fade-in duration-500">
      ...
      <div className="w-full max-w-[1600px] mx-auto space-y-8">
        ...
        {turmas.length === 0 ? (
          ...
          turmas.map(minhaTurma => {
            ...
            return (
              <div key={minhaTurma.nome} className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-50">
                
                {/* Header da Turma */}
                <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-6 mb-8 border-b border-slate-100 pb-8">
                  <div className="flex flex-col">
                    <h1 className="text-3xl md:text-5xl font-black text-slate-800 tracking-tighter m-0 flex items-center gap-4">
                      🏫 {minhaTurma.nome}
                    </h1>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                    <button 
                      ...
                      className="flex-1 lg:flex-none flex items-center justify-center gap-3 bg-indigo-600 text-white px-8 py-4 md:py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg hover:bg-indigo-700 transition-all active:scale-95"
                    >
                      <span className="text-xl">📝</span> Registrar Agenda
                    </button>
                    <button 
                      onClick={() => abrirAgendaTurma(minhaTurma, 'consultar')} 
                      className="flex-1 lg:flex-none flex items-center justify-center gap-3 bg-white text-indigo-600 border-2 border-indigo-100 px-8 py-4 md:py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:border-indigo-200 transition-all active:scale-95 shadow-sm"
                    >
                ...
                {/* Lista de Alunos Transformada em GRID (Lado a Lado) */}
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 md:gap-6">
                  {alunosTurma.map((aluno, index) => {
                    // Paleta Cíclica Exatamente como você pediu
                    const classesPaleta = [
                    ...
                    return (
                      <div 
                        ...
                        className={`p-5 md:p-6 rounded-[2rem] flex flex-row items-center justify-between cursor-pointer border-l-8 transition-all hover:-translate-y-1 hover:shadow-xl ${corSelecionada} gap-4 w-full`}
                      >
                        <div className="flex items-center gap-4 flex-1 overflow-hidden">
                          ...
                          <div className="flex flex-col justify-center flex-1 overflow-hidden">
                            <p className="font-black text-base md:text-lg m-0 leading-tight truncate" title={aluno.nome}>
                              {aluno.nome}
                            </p>
                            <div className="flex items-center flex-wrap gap-2 mt-2">
                              ...
                              {aluno.tem_alergia && (
                                <span className="text-[8px] md:text-[9px] bg-red-100 text-red-600 px-3 py-1 rounded-lg font-black uppercase tracking-widest shadow-sm whitespace-nowrap">
                                  ⚠️ Alergia
                                </span>
                              )}
                            </div>
