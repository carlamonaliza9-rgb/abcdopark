...
export default function AvancosDificuldadesPage() {
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-32 w-full px-4 md:px-8 py-6 min-h-screen bg-[#f4f7f9]">
      <div className="w-full max-w-[1500px] mx-auto space-y-6">
        
        {/* ============================================== */}
        {/* CABEÇALHO FILTROS */}
        {/* ============================================== */}
        <header className="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tighter m-0 flex items-center gap-3">
              <span className="bg-indigo-100 text-indigo-600 p-2.5 rounded-2xl"><BrainCircuit size={28} strokeWidth={2.5}/></span> 
              Avanços e Dificuldades
            </h1>
            <p className="text-[11px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">
              {ehAdmin ? "Acompanhamento Pedagógico Global" : `Registro de Pareceres Semestrais • Prof. ${nomeLogado}`}
            </p>
          </div>
          
          <div className="flex flex-col sm:flex-row gap-4 bg-slate-50 p-3 rounded-2xl border border-slate-100 w-full lg:w-auto">
            <div className="flex flex-col gap-1.5 flex-1 sm:w-56">
              ...
              <select 
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold outline-none focus:border-indigo-400 transition-colors shadow-sm"
              >
        ...
        {turmaSelecionada && alunos.length > 0 ? (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 relative">
            ...
            {alunos.map(aluno => {
              ...
              return (
                <div key={aluno.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-5 md:p-6 flex flex-col justify-between hover:shadow-md transition-all gap-5 relative overflow-hidden group">
                  
                  {/* Status superior de preenchimento */}
                  <div className="absolute top-0 right-0 m-5 flex items-center gap-1.5">
                    {foiAlterado ? (
                      ...
                      <span className="text-[9px] font-black uppercase bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-md flex items-center gap-1">
                        <CheckCircle2 size={10} strokeWidth={3}/> Preenchido
                      </span>
                    ) : (
                      <span className="text-[9px] font-black uppercase bg-rose-50 text-rose-500 px-2.5 py-1 rounded-md flex items-center gap-1 border border-rose-100">
                        <AlertCircle size={10} strokeWidth={3}/> Pendente
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 border-b border-slate-50 pb-4">
                    ...
                    <div className="flex flex-col pr-16">
                      <span className="text-base font-black text-slate-800 leading-tight truncate" title={aluno.nome}>
                        {aluno.nome}
                      </span>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Ficha do Estudante</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <textarea
                        ...
                        rows={5}
                        className="w-full p-3.5 text-xs font-semibold text-slate-600 placeholder-slate-400 bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:border-emerald-400 focus:bg-white transition-all resize-none leading-relaxed"
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <textarea
                        ...
                        rows={5}
                        className="w-full p-3.5 text-xs font-semibold text-slate-600 placeholder-slate-400 bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:border-rose-400 focus:bg-white transition-all resize-none leading-relaxed"
                      />
                    </div>
                  ...
                  <div className="flex justify-end border-t border-slate-50 pt-3 mt-1">
                    <button
          ...
          <div className="bg-white rounded-[2.5rem] p-12 text-center border border-slate-100 shadow-sm mt-6">
            <p className="text-xs font-black uppercase text-slate-400 tracking-widest m-0">
              {!turmaSelecionada 
