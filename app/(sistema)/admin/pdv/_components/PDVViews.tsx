...
export function HeaderPDV({ router }: any) {
  return (
    <div className="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-slate-200/60 flex justify-between items-center relative overflow-hidden">
      ...
      <button onClick={() => router.push('/admin/dashboard')} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 font-medium text-sm rounded-lg hover:bg-slate-50 shadow-sm transition-colors">
      ...
      {!alunoSelecionado ? (
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all shadow-sm font-medium" 
          />
          {buscaAluno && (
        ...
        <div className="flex justify-between items-center bg-indigo-50/50 p-3 rounded-xl border border-indigo-100">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-white text-indigo-600 rounded-lg shadow-sm border border-indigo-100 flex items-center justify-center text-lg font-bold">
              {alunoSelecionado.nome.charAt(0)}
            </div>
            <div>
              <h3 className="font-bold text-slate-800 text-sm leading-tight">{alunoSelecionado.nome}</h3>
              <span className="text-[10px] font-bold text-emerald-700 uppercase">
                Saldo Virtual: R$ {saldoAtualAluno.toFixed(2)}
              </span>
            </div>
          </div>
          <button onClick={() => setAlunoSelecionado(null)} className="text-xs bg-white border border-slate-200 text-slate-500 font-bold px-3 py-2 rounded-lg hover:text-rose-600 shadow-sm transition-colors">
            Trocar Cliente
          </button>
        </div>
      )}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 flex flex-col h-[400px]">
      ...
      <div className="space-y-3 overflow-y-auto custom-scrollbar pr-2 flex-1">
        {inadimplentesTop5.length > 0 ? inadimplentesTop5.map((item: any) => (
            <div key={item.alunoRaw.id} className="flex justify-between items-center p-3.5 bg-rose-50/50 border border-rose-100 rounded-xl">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setAlunoSelecionado(item.alunoRaw)} className="bg-rose-100 hover:bg-rose-600 hover:text-white text-rose-700 p-1.5 rounded-lg transition-colors">
...
export function StatusCaixaReduzido({ caixaAtual, setModalFechamentoAberto, carregarHistoricoCaixas, setModalMovimentacao }: any) {
  ...
  return (
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200/60 flex flex-col md:flex-row justify-between items-center gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="text-sm font-bold text-slate-800">Caixa Aberto</h4>
          <span className="text-xs text-slate-500 font-medium">Iniciado às {new Date(caixaAtual.data_abertura).toLocaleTimeString('pt-BR')}</span>
        </div>
      </div>
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setModalMovimentacao({aberto: true, tipo: 'suprimento'})} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-lg border border-emerald-100 hover:bg-emerald-100 transition-colors">Suprimento (+)</button>
        <button onClick={() => setModalMovimentacao({aberto: true, tipo: 'sangria'})} className="px-3 py-1.5 bg-rose-50 text-rose-700 text-xs font-bold rounded-lg border border-rose-100 hover:bg-rose-100 transition-colors">Sangria (-)</button>
        <button onClick={() => setModalFechamentoAberto(true)} className="px-3 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg hover:bg-slate-900 transition-colors">Fechar Caixa</button>
        <button onClick={carregarHistoricoCaixas} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 text-xs font-bold rounded-lg hover:bg-slate-50 transition-colors">Auditoria</button>
      </div>
    </div>
  );
}
...
export function AreaDeVendasComAbas({ alunoSelecionado, dividasAluno, carrinho, removerDoCarrinho, adicionarAoCarrinho, clean, novoItem, setNovoItem, uniformesTamanhos, setUniformesTamanhos, uniformesVenda, setUniformesVenda, totalVendaUniforme, lancarItemAvulsoNoCarrinho }: any) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden flex flex-col h-[520px] animate-in slide-in-from-bottom-4">
...
export function CarrinhoLateral({ caixaAtual, alunoSelecionado, carrinho, removerDoCarrinho, subtotalCarrinho, setModalCaixaAberto, abrirModalCheckout }: any) {
  return (
    <div className="xl:col-span-4 relative flex flex-col h-[520px]">
      {!caixaAtual && alunoSelecionado && (
        <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-10 rounded-2xl flex flex-col items-center justify-center border border-slate-200 shadow-xl">
          <div className="bg-white p-6 rounded-2xl shadow-2xl border border-slate-200 text-center max-w-[85%] animate-in zoom-in-95">
            ...
            <button onClick={() => setModalCaixaAberto(true)} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-md transition-all">Abrir Caixa</button>
          </div>
        </div>
      )}

      <div className={`bg-white p-5 rounded-2xl shadow-xl border border-slate-200 flex flex-col flex-1 border-t-4 border-t-indigo-500 ${!caixaAtual && alunoSelecionado ? 'opacity-30 pointer-events-none' : ''}`}>
        <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-100">
          <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">Resumo da Venda</h2>
          <span className="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md text-xs font-bold">{carrinho.length} Itens</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-2 mb-4">
          {carrinho.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center space-y-3 opacity-50">
              <svg className="w-10 h-10 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
              <p className="text-slate-500 font-bold text-xs">Carrinho Vazio</p>
            </div>
          ) : carrinho.map((item: any, idx: number) => (
            <div key={idx} className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100 group">
              <div className="truncate pr-2">
                <span className="text-xs font-bold text-slate-800 block truncate" title={item.descricao}>{item.descricao}</span>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-xs font-black text-slate-800">R$ {(item.valor_total - item.valor_pago).toFixed(2)}</span>
                <button onClick={() => removerDoCarrinho(item.id)} className="text-slate-400 hover:text-rose-600 w-6 h-6 flex items-center justify-center rounded"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-auto">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xl font-black text-slate-800">R$ {subtotalCarrinho.toFixed(2)}</span>
          </div>
          <button 
            onClick={abrirModalCheckout} 
            disabled={carrinho.length === 0 || !caixaAtual} 
            className={`w-full py-3.5 rounded-xl font-bold text-sm uppercase tracking-wider transition-all duration-200 shadow-md ${carrinho.length === 0 || !caixaAtual ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700 text-white'}`}
          >
            Ir para Pagamento
          </button>
        </div>
          ...
          <div className="flex-[2] bg-white border-l border-slate-100 flex flex-col">
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-200">
              ...
              <button 
                ...
                className={`w-full py-4 rounded-xl font-bold text-base uppercase tracking-wider transition-all duration-200 flex justify-center items-center gap-2 ${processando || (totalComAcrescimos > 0 && totalPagoRodada === 0 && clean(acrescimos.desconto) === 0) ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : (faltaPagar > 0 ? 'bg-amber-500 hover:bg-amber-600 text-white shadow-lg' : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg')}`}
              >
