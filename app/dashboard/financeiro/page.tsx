...
export default function FinanceiroPage() {
  ...
  async function zerarMes() {
    ...
    if (confirm("Resetar status dos alunos e zerar os cards e gráficos deste mês?")) {
      setCarregando(true);
      const [ano, mes] = mesFiltro.split('-');
      const dataInicio = `${ano}-${mes}-01`;
      const dataFim = `${ano}-${mes}-31`;

      // 1. Volta o status dos alunos para pendente
      await supabase.from('alunos').update({ status: 'pendente' }).not('id', 'is', null);

      // 2. Apaga pagamentos e gastos DO MÊS SELECIONADO para zerar os cards e gráficos
      ...
      // Recarrega os dados (isso zera automaticamente a tela com base no banco limpo)
      await carregarDados();
    }
  }
