...
export default function FechamentoLetivo() {
  ...
  async function atualizarAluno(id: string, campo: string, valor: any) {
    await supabase.from('alunos').update({ [campo]: valor }).eq('id', id);
    carregarDados();
  }
  ...
  async function promoverAprovados() {
    ...
    if (!confirm("Isso moverá todos os alunos 'APROVADOS' para a próxima série e resetará o financeiro para o novo ano. Deseja continuar?")) return;
      ...
      if (aluno.situacao_academica === 'Aprovado') {
        ...
        if (indexAtual !== -1 && indexAtual < turmasOrdem.length - 1) {
          ...
          await supabase.from('alunos').update({ 
            ...
            status: 'pendente' // Reseta para cobranças do novo ano
          }).eq('id', aluno.id);
        }
      }
    }
    ...
    <div style={{ width: '100%', padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f3f4f6', minHeight: '100vh' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          ...
          <p style={{ color: '#6b7280' }}>Gerencie as médias anuais e a transição de turmas.</p>
        </div>
        <button 
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                  <tr style={{ fontSize: '12px', color: '#6b7280', borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <th style={{ padding: '12px', width: '150px' }}>MÉDIA ANUAL</th>
                    ...
                    <tr key={aluno.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                      ...
                      <td style={{ padding: '12px' }}>
                        <input 
                          type="number" 
                          step="0.1"
                          defaultValue={aluno.media_anual} 
                          onBlur={(e) => atualizarAluno(aluno.id, 'media_anual', e.target.value)}
                          style={{ width: '80px', padding: '8px', borderRadius: '8px', border: '1px solid #e2e8f0', textAlign: 'center', fontWeight: 'bold' }}
                        />
                      </td>
                      <td style={{ padding: '12px' }}>
                        <select 
