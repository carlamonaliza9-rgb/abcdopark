...
export default function DashboardProfessorPage() {
  ...
  const getEventoStyle = (titulo: string) => {
    ...
    return { 
      ...
      border: isEspecial ? "border-purple-500" : "border-blue-600", 
      color: isEspecial ? "text-purple-700" : "text-blue-700" 
    };
  };

  if (carregando) return <div className="p-10 text-center font-black uppercase text-slate-300 animate-pulse tracking-widest">Sincronizando painel...</div>;

  return (
    <div className="w-full min-h-screen bg-slate-50/50 p-4 md:p-8 font-sans animate-in fade-in duration-700">
      
      {/* MODAL CONFIGURAÇÕES */}
      {modalConfigAberto && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl animate-in zoom-in-95">
            <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight mb-6">Configurações de Perfil</h2>
            <div className="mb-6">
            ...
            <div className="flex gap-4">
              <button onClick={() => setModalConfigAberto(false)} className="flex-1 py-4 rounded-2xl border border-slate-200 text-slate-500 font-black uppercase tracking-wider hover:bg-slate-50 transition-colors">Cancelar</button>
              <button onClick={atualizarPerfil} className="flex-1 py-4 rounded-2xl bg-indigo-600 text-white font-black uppercase tracking-wider shadow-lg hover:bg-indigo-700 transition-all">Salvar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalBdayAberto && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-8">
            <div className="bg-gradient-to-br from-indigo-600 to-purple-600 p-8 text-center text-white relative">
              ...
              {aniversariantesHoje.some(p => p.email === userEmail) ? (
                <div className="mt-4 px-4 text-indigo-50">
                  ...
                  <p className="mt-4 text-xs font-black uppercase tracking-widest opacity-90">— Um abraço bem apertado da Família ABC DO PARK ❤️</p>
                </div>
              ) : (
                <p className="text-indigo-100 font-medium text-sm md:text-base mt-2">Hoje o dia é de festa e gratidão na nossa escola!</p>
              )}
            </div>
            ...
            <div className="p-6 md:p-8 bg-slate-50/50">
              <div className="flex flex-col gap-4">
                {aniversariantesHoje.map(pessoa => {
                  ...
                  return (
                    <div key={`${pessoa.tipo}-${pessoa.id}`} className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${ehVoce ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-100'}`}>
                      <div className="w-16 h-16 rounded-full bg-white border border-slate-200 overflow-hidden flex-shrink-0 shadow-sm">
                        {pessoa.foto_url ? (
                          <img src={pessoa.foto_url} className="w-full h-full object-cover" alt="" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center font-black text-slate-400 text-xl">{pessoa.nome.charAt(0)}</div>
                        )}
                      </div>
                      <div className="flex-1">
                        <h4 className="text-base md:text-lg font-black text-slate-800 line-clamp-1">
                          {ehVoce ? "Você está de parabéns! 🥳" : pessoa.nome}
                        </h4>
                        <span className={`text-[10px] font-black uppercase tracking-widest ${ehVoce ? 'text-blue-600' : (pessoa.tipo === 'funcionario' ? 'text-purple-600' : 'text-slate-500')}`}>
                      ...
                      {!ehVoce && (
                        <button 
                          ...
                          className="w-12 h-12 rounded-2xl bg-green-500 hover:bg-green-600 text-white flex items-center justify-center text-xl shadow-md transition-transform active:scale-95"
              ...
              <button 
                ...
                className="w-full mt-8 py-4 rounded-2xl bg-slate-800 hover:bg-slate-900 text-white font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg"
              >
      ...
      <header className="flex justify-end items-center mb-6">
        <button 
          ...
          className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase tracking-wider text-xs md:text-sm shadow-md transition-all active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {/* CARDS PROFESSOR - BLOCO ESQUERDO */}
        <div className="lg:col-span-7 grid grid-cols-1 gap-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 flex flex-col items-center text-center relative overflow-hidden">
            <div className="flex justify-center items-center gap-3 w-full mb-2 relative z-10">
              <h1 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tight m-0">Olá, {nomeUsuario}! 👋</h1>
              <button 
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6 z-10">Resumo atualizado da ABC DO PARK.</p>
            
            <div className="w-full max-w-[280px] mb-8 z-10">
              <img 
                src={ilustracaoProfessor || "/image_de2d33.jpg"} 
                alt="Ilustração" 
                className="w-full h-auto rounded-3xl object-contain drop-shadow-sm" 
              />
            </div>
            
            <div className="w-full border-t border-slate-50 pt-6 mt-auto z-10">
              <h3 className="text-6xl font-black text-slate-800 m-0 leading-none">{dados.totalAlunos}</h3>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2">Meus Alunos</p>
            </div>
          </div>

        </div>

        {/* COLUNA DIREITA - EVENTOS, NIVER E SAÚDE */}
        <div className="lg:col-span-5 flex flex-col gap-8">
          
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-100">
            <h2 className="text-xl md:text-lg font-black text-slate-800 uppercase tracking-tighter italic flex items-center gap-2 mb-6">
            ...
            <div className="flex flex-col gap-3">
              {dados.proximosEventos.length > 0 ? dados.proximosEventos.map((ev, i) => {
                ...
                return (
                  <div key={i} className={`p-4 rounded-2xl border-l-4 ${estilo.bg} ${estilo.border}`}>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${estilo.color}`}>
                      {formatarDataLocal(ev.data)}
                    </span>
                    <p className="mt-1 text-sm font-bold text-slate-800 leading-snug">{ev.titulo}</p>
                  </div>
                );
              }) : (
                <div className="p-6 text-center border-2 border-dashed border-slate-100 rounded-2xl">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Nenhuma programação agendada.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-100">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
              {dados.aniversariantes.length > 0 ? dados.aniversariantes.map(persona => {
                ...
                return (
                  <div key={`${persona.tipo}-${persona.id}`} className="text-center min-w-[80px] flex flex-col items-center group">
                    <div className={`w-16 h-16 rounded-full bg-slate-50 border-2 p-0.5 overflow-hidden transition-transform group-hover:scale-105 ${isFunc ? 'border-purple-400' : 'border-blue-400'}`}>
                      <div className="w-full h-full rounded-full overflow-hidden bg-white flex items-center justify-center">
                        {persona.foto_url ? (
                          <img src={persona.foto_url} className="w-full h-full object-cover" alt="" />
                        ) : (
                          <span className="font-black text-slate-300 text-lg">{persona.nome.charAt(0)}</span>
                        )}
                      </div>
                    </div>
                    <p className="text-xs font-bold mt-2 text-slate-700 line-clamp-1">{persona.nome.split(' ')[0]}</p>
                    <p className={`text-[10px] font-black uppercase tracking-widest ${isFunc ? 'text-purple-500' : 'text-blue-500'}`}>
                      Dia {dia < 10 ? `0${dia}` : dia}
                    </p>
                  </div>
                );
              }) : (
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-300 w-full text-center py-4">Nenhum aniversário este mês.</p>
              )}
            </div>
          </div>

          <div className="bg-rose-50/50 rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-rose-100">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              <h2 className="text-xl md:text-lg font-black text-rose-600 uppercase tracking-tighter italic flex items-center gap-2">
              ...
              <input 
                ...
                className="px-4 py-2 rounded-xl border border-rose-200 text-xs font-bold text-slate-600 w-full sm:w-32 outline-none focus:border-rose-400 bg-white"
              />
            </div>
            <div className="flex flex-col gap-3 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
              {alertasFiltrados.length > 0 ? alertasFiltrados.map(aluno => (
                <div key={aluno.id} className="bg-white p-3 rounded-2xl border border-rose-100 flex items-center gap-3 shadow-sm hover:border-rose-200 transition-colors">
                  <div className="w-10 h-10 rounded-full bg-rose-50 border border-rose-100 flex-shrink-0 flex items-center justify-center overflow-hidden">
                    {aluno.foto_url ? (
                      <img src={aluno.foto_url} className="w-full h-full object-cover" alt="" />
                    ) : (
                      <span className="text-rose-300 font-black text-sm">{aluno.nome.charAt(0)}</span>
                    )}
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-slate-700 leading-tight line-clamp-1">{aluno.nome}</span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-rose-500 line-clamp-1">{aluno.alergia_descricao || "Alergia registrada"}</span>
                  </div>
                </div>
              )) : (
                <p className="text-[10px] font-black uppercase tracking-widest text-rose-300/80 w-full text-center py-4">Nenhum alerta encontrado.</p>
              )}
            </div>
          </div>
      ...
      {modalCalendarioAberto && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-slate-50 rounded-[2.5rem] w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center p-6 sm:p-8 bg-white border-b border-slate-100 shadow-sm z-10">
                ...
                {meses.map((mesNome, index) => {
                  ...
                  return (
                    <div key={index} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col">
                      <h3 className="text-sm font-black text-indigo-600 uppercase tracking-widest border-b-2 border-slate-50 pb-4 mb-4">{mesNome}</h3>
                          ...
                          eventosDoMes.map((ev, i) => {
                            ...
                            return (
                              <div key={i} className={`p-3 rounded-xl border-l-4 flex flex-col gap-1 ${estilo.bg} ${estilo.border}`}>
                                <span className={`font-black text-[10px] uppercase tracking-widest ${estilo.color}`}>
                                  Dia {extrairDiaUTC(ev.data)}
                                </span>
                                <span className="font-bold text-slate-700 leading-tight text-xs">{ev.titulo}</span>
                              </div>
                            );
                          })
                        ) : (
                          <div className="flex-1 flex items-center justify-center min-h-[60px] border-2 border-dashed border-slate-50 rounded-xl">
      ...
      {/* Adicionando um pequeno estilo global para barra de rolagem */}
